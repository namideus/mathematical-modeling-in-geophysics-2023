...
[xx, yy] = meshgrid(x, y);
% v-component
V = (D_1.*r_l.*sin(r_l.*xx)-D_2.*r_l.*cos(r_l.*xx)-A.*C_1.*exp(A.*xx)-B.*C_2.*exp(B.*xx))*sin(q_m.*yy)+ (D_1_bar.*r_s.*sin(r_s.*xx)-D_2_bar.*r_s.*cos(r_s.*xx)-A_bar.*C_1_bar.*exp(A_bar.*xx)-B_bar.*C_2_bar.*exp(B_bar.*xx)).*sin(q_p.*yy);
% u-component
